...
@section('title','新增文章 - 文章管理')

@section('content')
<article class="page-container">
	<form class="form form-horizontal" id="form-article-add" action="{{url('admin/article_store')}}" method="post">
	{{csrf_field()}}
		<div class="row cl">
			...
			@endif
				<input type="text" class="input-text" value="{{old('title')}}" placeholder="" id="articletitle" name="title">
			</div>
		</div>
		<div class="row cl">
			...
			@endif
				...
				<input type="text" class="input-text" value="{{old('editor')}}" placeholder="" id="author" name="editor">
			</div>
		</div>
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
			 ...
			 <span class="select-box">
				<select name="cate_id" class="select">
					...
					<option value="{{$data->id}}">{{str_repeat('——',$data->heng)}}{{$data->cate_name}}</option>
					@endforeach
				</select>
				</span> </div>
		</div>
		<div class="row cl">
			...
			@endif
				<textarea name="description" cols="" rows="" class="textarea"  placeholder="说点什么..." datatype="*10-100" dragonfly="true" nullmsg="备注不能为空！" >{{old('description')}}</textarea>
			</div>
		</div>
		<div class="row cl">
			...
			@endif
				<input type="text" class="input-text" value="{{old('tags')}}" placeholder="" id="tags" name="tags">
			</div>
		</div>
        ...
        <div id="uploader-demo" class="formControls col-xs-8 col-sm-9">
            ...
            <!-- 用来存放上传成功后的图片路径 -->
            <input type="hidden" name="thumb_pic" value="{{old('thumb_pic')}}">
		...
		<div class="row cl">
			...
			@endif
				<script id="editor" name="content"  type="text/plain" style="width:100%;height:400px;">{!!old('content')!!}</script>
			</div>
		</div>
		...
		<div class="row cl">
					...
					@endif
						<input type="text" class="input-text" value="100" placeholder="" id="" name="article_order">
					</div>
					<div class="col-3">
					</div>
				</div>
